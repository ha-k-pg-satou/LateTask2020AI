...
void Graphics::DrawFan(Vec2 center_pos, float arc_degree, float length, float rotate_degree, DWORD color)
{
	struct CircleVertex
	{
	...
	m_D3DDevice->SetTexture(0, nullptr);
	m_D3DDevice->DrawPrimitiveUP(D3DPT_TRIANGLEFAN, vertices.size() - 2, &vertices[0], sizeof(CircleVertex));
}
